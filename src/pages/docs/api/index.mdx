  ...
  useEffect(() => {
    ...
    const loadSwaggerUI = async () => {
      ...
      const ui = SwaggerUIBundle({
        ...
        validatorUrl: null, // Disable validator for better compatibility
        oauth2RedirectUrl: `${origin}/docs/api/oauth2-redirect.html`,
        requestInterceptor: (request) => {
          // Add CORS headers for testing
          request.headers['Access-Control-Allow-Origin'] = '*';
          request.headers['Access-Control-Allow-Methods'] = 'GET, POST, PUT, DELETE, PATCH, HEAD, OPTIONS';
          request.headers['Access-Control-Allow-Headers'] = 'Content-Type, Authorization, X-Requested-With';
