...
import { getCommitteeDirectory } from '@/actions/committee-actions';
...
export default async function CommitteePublicPage({
  ...
  const prefill = onboarding?.data || (onboarding?.userEmail ? { email: onboarding.userEmail } : null);
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      ...
      {currentCommittee ? (
        <section className="space-y-4">
          ...
          {isLoggedIn ? (
            <CommitteeApplyForm committeeId={currentCommittee.id} prefill={prefill} isLoggedIn={isLoggedIn} />
          ) : (
